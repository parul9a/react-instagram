...
function Sidebar() {
    const sidebarItems = [
        ...
        {    icon: <Avatar size={"sm"} src='/profilepic.png' name='Burak Orkmez'/>, text:"Profile", link:"/asaprogrammer", }
    ]
            ...
            <Flex direction={"column"} gap={5} cursor={"pointer"}>
                {sidebarItems.map((item,index) => (
                    <Tooltip key={index} hasArrow label={item.text} placement='right' ml={1} openDelay={500} display={{base:"block",md: "none"}}>
                        <Link as={RouterLink} to={item.link || null} display={'flex'} alignItems={"center"} gap={4} _hover={{bg:"whiteAlpha.400"}} borderRadius={6} p={2} w={{base:10,md:"full"}} justifyContent={{base:"center", md: "flex-start"}}>
                            {item.icon}
